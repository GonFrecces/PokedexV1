...
import { Image, useDisclosure, Snippet } from "@nextui-org/react";
...
const PokemonCard = ({ pokemon }) => {
    ...
    return (
        <div className="overflow-visible relative" style={{ cursor: 'pointer' }} onClick={() => {setSelected(pokemon); onOpen();}}>
            <div className="absolute top-2 right-2 z-20">
                <Snippet size="sm" hideCopyButton symbol="#" variant="bordered" className='dark:bg-slate-800 bg-white'>    
                    <span className="text-tiny font-bold">{pokemon.order}</span>
                </Snippet>
            </div>
            <div className="absolute inset-0 rounded-xl transition-all duration-300" style={getBackgroundStyle(pokemon.types)}/>
